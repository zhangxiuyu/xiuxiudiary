<template>
	<view class="container">
		...
		<!-- 	<swiper class="card-swiper" :class="dotStyle?'square-dot':'round-dot'" :indicator-dots="true" :circular="true"
			...
			<!-- 电影列表 -->
			<view class="movieH">最新电影</view>
			<view class="movieBox">
				<view v-for="(item, index) in movieInfo" :key="index" class="movieList" @click="getDate(item)">
					<image :src="item.cover_url" class="movieImg"></image>
					<view>
						{{ item.title | ellipsis }}
						<view class="moiveRate">
							
							<text  class="moiveRateT">{{item.update_date?item.update_date.substring(0,10):''}}</text>
						</view>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// }, 10)
			uni.request({
				// url: 'http://127.0.0.1:5000/video_list',
				url: 'http://101.201.150.84:5000/video_list',
				header: {
				...
				success: (res) => {
					...
					console.log(this, 'this.movieInfo ')
				}
			})
		},
		data() {
		...
		methods: {
			...
			getmorenews() {
				...
				uni.request({
					...
					success: function(res) {
						...
						that.movieInfo = that.movieInfo.concat(res.data);
			...
			getDate(item) {
				...
				uni.navigateTo({
					url: '../video/detail?id=' + item._id.$oid,
					animationType: 'pop-in',
					animationDuration: 200
				})
			},
